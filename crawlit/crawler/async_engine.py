...
class AsyncCrawler:
    ...
    using asynchronous IO for better performance.
    ...
    def __init__(self, start_url, max_depth=3, internal_only=True, 
                 ...
                 enable_table_extraction=False, max_concurrency=10):
        """Initialize the async crawler with given parameters.
        ...
        self.processing_urls = set()  # URLs currently being processed
    ...
    async def crawl(self):
        ...
        # Use a list to keep track of all running tasks
        tasks = []
        
        # Process URLs from the queue until it's empty and all tasks are done
        while self.queue or tasks:
            # Start new tasks up to the concurrency limit
            while self.queue and len(tasks) < self.max_concurrency:
                current_url, depth = self.queue.popleft()
                
                # Skip if we've already visited or are currently processing this URL
                if current_url in self.visited_urls or current_url in self.processing_urls:
                    continue
                ...
                self.processing_urls.add(current_url)
                
                # Create a task for processing this URL
                task = asyncio.create_task(self._process_url(current_url, depth))
                tasks.append(task)
            
            # If there are no tasks to wait for, break out of the loop
            if not tasks:
                break
    ...
    async def _process_url(self, current_url, depth):
        """Process a single URL: fetch and extract links asynchronously"""
        async with self.semaphore:
            ...
            # Fetch the page using our async fetcher
            ...
            self.processing_urls.remove(current_url)
            ...
            if success:
                ...
                try:
                    ...
                    if 'text/html' in content_type and response.text is not None:
                        ...
                        logger.debug(f"Processing binary content ({content_type}) at {current_url}")
                        self.results[current_url]['is_binary'] = True
                        self.results[current_url]['binary_content_length'] = len(response.content) if hasattr(response, 'content') else 0
                    else:
    ...
    async def _should_crawl(self, url):
        """Determine if a URL should be crawled based on settings (async version)"""
        # Check if URL is already visited or being processed
        if url in self.visited_urls or url in self.processing_urls:
            logger.debug(f"Skipping already visited/processing URL: {url}")
            return False
